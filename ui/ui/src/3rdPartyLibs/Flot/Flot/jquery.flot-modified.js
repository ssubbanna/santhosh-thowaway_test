...
(function ($) {
  ...
  function Plot(placeholder, data_, options_, plugins) {
    ...
    plot.findPointsByTimestamp = findPointsByTimestamp;
    ...
    function bindEvents() {
      ...
      if (plot.getOptions().crosshair.mode) {
        $('body').bind('plot-highlighted', function (e, data) {
          //determine if this plot is visible, if not then don't draw the tooltip
          var element = document.elementFromPoint(plot.offset().left, plot.offset().top);
          if (!element || element.className != 'flot-overlay') return;

          var pts = plot.findPointsByTimestamp(data.point[0]);
          if (data.plot !== plot) plot.unhighlight(null, null, true);
          _.each(pts, function (pt) {
            if (data.plot !== plot) plot.highlight(pt.series, pt.datapoint, false, true);
            var pos = {};
            if (data.plot !== plot) {
              pos.x = pt.datapoint[0];
              pos.y = pt.datapoint[1];
            ...
            plot.setTooltipPosition(pos, pt.seriesIndex);
            plot.showTooltip(pt, pos);
          });
        });

        $('body').bind('plot-unhighlighted', function (e, data) {
          if (data.plot !== plot) {
            plot.clearCrosshair();
            plot.unhighlight(null, null, true);
          }
        });
      }
    ...
    function findPointsByTimestamp(ts) {
      var points = plot.getData();
      var ret = [];
      for (var i = 0; i < points.length; i++) {
        for (var j = 0; j < points[i].data.length; j++) {
          if (_.isArray(points[i].data[j]) && points[i].data[j][0] == ts) {
            var item = {
              ...
              seriesIndex: i,
              series: points[i],
              seriesLabel: points[i].label
            }
            var offset = plot.offset();
            item.pageX = offset.left + item.x;
            item.pageY = offset.top + item.y;
            ret.push(item);
          }
        }
      }
    ...
    function highlight(s, point, auto, quiet) {
      if (typeof s == "number")
        s = series[s];
      ...
      if (i == -1) {
        ...
        if (!quiet) {
          $('body').trigger('plot-highlighted', {plot: this, point: point});
        }
        triggerRedrawOverlay();
      }
      else if (!auto)
    ...
    function unhighlight(s, point, quiet) {
      if (highlights.length == 0) return;

      if (s == null && point == null) {
        highlights = [];
        if (!quiet) $('body').trigger('plot-unhighlighted', {plot: plot});
        plot.removeTooltip();
        triggerRedrawOverlay();
        return;
      }
